...
(async()=>{
 ...
 for(const check of contract.checks){const viewport=check.device==='mobile'?{width:390,height:844}:{width:1440,height:1000};const context=await browser.newContext({viewport});const page=await context.newPage();const consoleErrors=[],failedRequests=[];page.on('console',m=>{if(m.type()==='error')consoleErrors.push(m.text())});page.on('requestfailed',r=>{try{const u=new URL(r.url());if(u.origin===new URL(base).origin)failedRequests.push(`${u.pathname}:${r.failure()?.errorText||'failed'}`)}catch{}});let failures=[];try{const resp=await page.goto(new URL(check.route,base).toString(),{waitUntil:'networkidle',timeout:45000});failures=await assertPage(page,check,resp,consoleErrors,failedRequests);}catch(e){failures=[e.message];}results.push({...check,status:failures.length?'FAIL':'PASS',failures});await context.close();}
