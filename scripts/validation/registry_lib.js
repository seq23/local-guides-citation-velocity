...
function sourceSnapshot(reg,previous=null){const snap=new Map();for(const rel of reg.lineage_policy?.source_roots||[]){const abs=path.join(ROOT,rel);for(const file of walk(abs)){const r=norm(path.relative(ROOT,file));if(/(^|\/)node_modules\//.test(r))continue;if(/(^|\/)[^/]+\.tmp-[0-9]+$/.test(r))continue;const sig=fileStatSig(file);const prior=previous&&previous.get(r);const hash=prior&&prior.sig===sig?prior.hash:fileSha(file);snap.set(r,{sig,hash});}}return snap;}
