...
if(reg){
 ...
 // 169 blocking validators turned main red on content motion almost daily; 36 guarded a real harm.
 const hardFailBases=reg.common_sense_validation_policy?.hard_fail_bases||{};const MAX_HARD_FAIL=40;
 check(Object.keys(hardFailBases).length>0,'registry:hard-fail-bases-missing');
 const activeHard=(reg.validators||[]).filter(v=>v.status==='ACTIVE'&&v.severity==='HARD_FAIL');
 check(activeHard.length<=MAX_HARD_FAIL,`registry:too-many-blockers:${activeHard.length}>${MAX_HARD_FAIL}: every blocker is a way for main to go red. Retire or demote one before adding another; raising MAX_HARD_FAIL is a deliberate decision, not a fix.`);
 ...
 for(const v of reg.validators||[]){check(v.id&&!ids.has(v.id),`registry:duplicate-id:${v.id}`);ids.add(v.id);check(v.path&&!paths.has(v.path),`registry:duplicate-path:${v.path}`);paths.add(v.path);check(severities.has(v.severity),`registry:severity:${v.id}`);check(scopes.has(v.scope),`registry:scope:${v.id}`);check(statuses.has(v.status),`registry:status:${v.id}`);check(fs.existsSync(path.join(ROOT,v.path)),`registry:missing-path:${v.id}:${v.path}`);check(Number.isInteger(v.timeout_seconds)&&v.timeout_seconds>0,`registry:timeout:${v.id}`);check(typeof v.risk_prevented==='string'&&v.risk_prevented.length>20,`registry:risk:${v.id}`);check(typeof v.why_existing_checks_do_not_cover==='string'&&v.why_existing_checks_do_not_cover.length>20,`registry:coverage-reason:${v.id}`);
  ...
  if(v.severity==='HARD_FAIL'&&v.status==='ACTIVE')check(Boolean(hardFailBases[v.hard_fail_basis]),`registry:hard-fail-without-basis:${v.id}: a blocking validator must name the harm it guards as hard_fail_basis, one of ${Object.keys(hardFailBases).join(', ')}. If it guards none, it is not a blocker: make it a warning or retire it.`);
  if(v.status==='RETIRED'){check(Array.isArray(v.replacement_ids),`registry:retired-replacement:${v.id}`);check(Boolean(v.retirement_reason),`registry:retired-reason:${v.id}`);check((v.profiles||[]).length===0,`registry:retired-profile:${v.id}`);}else{check((v.profiles||[]).length>0,`registry:active-profile:${v.id}`);for(const p of v.profiles||[])check(Boolean(reg.profiles[p]),`registry:unknown-profile:${v.id}:${p}`);}
