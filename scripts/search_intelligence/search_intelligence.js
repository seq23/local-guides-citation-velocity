...
function initEmpty(){for(const [rel,value] of Object.entries({'data/search_intelligence/repair_receipts.json':{schema_version:'1.0',repairs:[]},'data/search_intelligence/retest_queue.json':{schema_version:'1.0',items:[]},'data/search_intelligence/retest_outcomes.json':{schema_version:'1.0',outcomes:[]},'data/search_intelligence/regression_ledger.json':{schema_version:'1.0',events:[]},'data/search_intelligence/verified_external_citations.json':{schema_version:'1.0',events:[]},'data/search_intelligence/automation_health.json':{schema_version:'1.1',state:'UNPROVEN',lanes:{},lanes_observed:0,red_lanes:[],last_validated_sha:null,last_failure_sha:null,last_recovery_sha:null,truth_rule:'CI health changes only from observed exact-SHA workflow results, recorded per lane. The top-level state is a rollup: RED if any lane is red, RECOVERED only when every observed lane is.'}}))if(!exists(rel))writeJson(rel,value);}
function full({apply=false}={}){const protectedBefore=protectedAgentSnapshot();let result;try{initEmpty();pickTargets();buildAgentBridge();buildProviderTruth();diagnose();buildCandidates();if(apply)applyRepairs();evaluateRetests();feedback();scorecard();result={targets:readJson('data/search_intelligence/target_query_set.json').count,agent_signals:readJson('data/search_intelligence/agent_signal_bridge.json').count,candidates:readJson('data/search_intelligence/repair_candidates.json').count};}finally{assertProtectedAgentSnapshotUnchanged(protectedBefore,protectedAgentSnapshot());}return result;}
