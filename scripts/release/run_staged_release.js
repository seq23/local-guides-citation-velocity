...
const SHARD=(()=>{const v=String(process.env.VALIDATION_SHARD||'').trim();if(!v)return null;if(!/^\d+\/\d+$/.test(v))throw new Error(`INVALID_VALIDATION_SHARD:${v} (expected I/N)`);return v;})();
const MERGE=(()=>{const v=String(process.env.VALIDATION_MERGE_SHARDS||'').trim();if(!v)return null;if(!/^\d+$/.test(v)||Number(v)<1)throw new Error(`INVALID_VALIDATION_MERGE_SHARDS:${v} (expected N >= 1)`);if(SHARD)throw new Error('VALIDATION_SHARD and VALIDATION_MERGE_SHARDS are mutually exclusive');return Number(v);})();
const STOP_AFTER=SHARD?'release-validation':null;
const stages=[];
...
stages.push(
 ...
 // SHARDED MERGE GATE. Validate Repo runs this pipeline on N parallel runners with
 // VALIDATION_SHARD=I/N: each runs every stage up to and including a 1/N slice of the
 // release profile (run_validation_registry.js --shard, dependency closure included)
 // and stops. One aggregate job then runs it with VALIDATION_MERGE_SHARDS=N: the
 // release-validation stage becomes merge_validation_shards.js, which proves every
 // validator of the profile was assigned exactly once and passed, and the tail
 // stages (advisory, evidence, manifest, attestation) run once on the merged
 // result. Unset, this is the single-runner pipeline it always was.
 stage('release-validation',SHARD?`node scripts/validation/run_validation_registry.js --profile release --shard ${SHARD}`:MERGE?`node scripts/validation/merge_validation_shards.js --count ${MERGE} --profile release`:'node scripts/validation/run_validation_registry.js --profile release',35),
...
(async()=>{fs.mkdirSync(LOG_ROOT,{recursive:true});const initial=fingerprint();let checkpoint=resume?safeRead(CHECKPOINT):null;let completed=[];if(checkpoint&&checkpoint.mode===mode&&checkpoint.status!=='PASS'&&checkpoint.status!=='SHARD_PASS'&&!checkpoint.shard&&checkpoint.source_fingerprint===initial){completed=checkpoint.completed_stages||[];console.log(`RESUMING RELEASE PIPELINE after ${completed.length} completed stage(s).`);}else if(checkpoint&&resume){console.log('RELEASE PIPELINE CHECKPOINT INVALIDATED BY SOURCE CHANGE; restarting safely.');}
