...
const scopeContract=read('data/overhaul/full_scope_overhaul_contract.json');
const minimums=scopeContract.minimums||{};
...
const stateRows=stateSources.states||[];
const derivedAuthorityCount=stateRows.reduce((n,state)=>n+(state.authorities||[]).length,0);
if(stateSources.count!==stateRows.length)errors.push(`state authority metadata mismatch: ${stateSources.count} vs ${stateRows.length}`);
if(stateRows.length<50)errors.push(`state coverage below 50: ${stateRows.length}`);
if(stateSources.authority_record_count!==derivedAuthorityCount)errors.push(`authority metadata mismatch: ${stateSources.authority_record_count||0} vs ${derivedAuthorityCount}`);
const authorityByRoute=new Map();
for(const state of stateSources.states||[]){
  if(state.authority_count!==(state.authorities||[]).length)errors.push(`authority count metadata mismatch for ${state.state}`);
  if((state.authorities||[]).length<8)errors.push(`state authority family coverage below baseline for ${state.state}`);
...
if(specs.count!==specs.pages.length)errors.push(`Velocity page spec metadata mismatch: ${specs.count} vs ${specs.pages.length}`);
const minVelocitySpecs=Number(minimums.velocity_state_and_support_pages||scopeContract.counts?.velocity_state_and_support_pages||0);if(specs.pages.length<minVelocitySpecs)errors.push(`Velocity page specs below approved baseline: ${specs.pages.length}<${minVelocitySpecs}`);
...
for(const page of specs.pages){
  ...
  if((page.sections||[]).length<3)errors.push(`fewer than three substantive decision sections ${page.slug}`);
  ...
  if(page.state){
    ...
    if(!Number.isInteger(page.self_healing?.projected_word_count)||page.self_healing.projected_word_count<250)errors.push(`projected content is materially thin ${page.slug}`);
    for(const section of page.sections||[]){
      faqCount++;
      if(!section.a||section.a.split(/\s+/).filter(Boolean).length<20)errors.push(`decision answer is materially incomplete ${page.slug}:${section.q}`);
...
const expectedStatePages=specs.pages.filter((page)=>page.state).length;
const expectedFaqs=specs.pages.filter((page)=>page.state).reduce((n,page)=>n+(page.sections||[]).length,0);
if(statePageCount!==expectedStatePages)errors.push(`state page registry parity mismatch: ${statePageCount} vs ${expectedStatePages}`);
...
for(const p of programs){if((p.sections||[]).length<3)errors.push(`fewer than three substantive decision sections ${p.slug}`); if(!p.dated_primary_fact)errors.push(`dated fact missing ${p.slug}`); if(!(p.source_urls||[]).length)errors.push(`source URLs missing ${p.slug}`);}
...
for(const [name,re] of [['vertical selector',/<section[^>]+id=["']vertical-routes["']/],['coverage',/<section[^>]+class=["'][^"']*coverage-panel/],['operations',/<section[^>]+class=["'][^"']*operations/],['methodology',/href=["']\/methodology(?:\.html)?["']/]]) if(!re.test(home)) errors.push(`homepage structure missing: ${name}`);
...
const evidence={validator:'velocity-only-overhaul',status:errors.length?'FAIL':'PASS',errors,advisories:[diversityAdvisory].filter(Boolean),counts:{velocity_specs:specs.pages.length,state_pages:statePageCount,state_authority_records:stateSources.authority_record_count,state_decision_sections:faqCount,exact_state_answers:exactAnswers.size,normalized_state_answers:normalizedAnswers.size,program_pages:programs.length,rendered_html:htmlFiles.length}}; fs.mkdirSync(path.join(ROOT,'artifacts/validation'),{recursive:true}); fs.writeFileSync(path.join(ROOT,'artifacts/validation/velocity-only-overhaul.json'),JSON.stringify(evidence,null,2)+'\n');
if(errors.length){console.error(errors.slice(0,80).join('\n'));process.exit(1);} console.log(`Velocity-only overhaul validation PASS: ${statePageCount} state pages, ${exactAnswers.size} exact decision answers, ${normalizedAnswers.size} normalized patterns`);
