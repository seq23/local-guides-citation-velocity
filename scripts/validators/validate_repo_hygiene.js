...
const updaterRequired=new Set((contract.updater_required_runtime_artifacts||[]).map(x=>String(x).replace(/\/$/,'')));
const requiredExclusions=['.git','node_modules','.build','logs','artifacts/validation/runtime','*.log','*.zip'];
if(!updaterRequired.has('dist')) requiredExclusions.push('dist');
