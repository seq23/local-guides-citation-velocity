...
const rendered = [];
const requiredSchema = ['Article', 'FAQPage', 'HowTo', 'BreadcrumbList'];
...
for (const entry of routes) {
  ...
  if (description.length < 60) pageErrors.push('missing_or_short_meta_description');
  if (h1.length < 6) pageErrors.push('missing_or_short_h1');
  ...
  if (answerWords < 8 || answerWords > 70) pageErrors.push(`direct_answer_word_count:${answerWords}`);
  ...
  if (!/<table\b/i.test(html) && !/<ol\b/i.test(html)) pageErrors.push('missing_table_or_numbered_list');
  ...
  if (siblings < 5 || siblings > 10) pageErrors.push(`sibling_link_count:${siblings}`);
  if (wordCount(html) < 180) pageErrors.push(`page_too_thin:${wordCount(html)}`);
