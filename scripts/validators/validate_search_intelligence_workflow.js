...
}else if(files.length!==8)errors.push(`workflow_count:${files.length}!=8`);const need=['search-intelligence-loop.yml','ci-health-recovery.yml'];for(const f of need)if(!files.includes(f))errors.push(`missing:${f}`);function text(f){return fs.readFileSync(path.join(dir,f),'utf8');}if(files.includes('search-intelligence-loop.yml')){const t=text('search-intelligence-loop.yml');for(const x of ['schedule:','workflow_dispatch:','npm run search:intelligence:closed-loop','npm run validate:search-intelligence'])if(!t.includes(x))errors.push(`search_workflow_missing:${x}`);}// The postdeploy public audit lane was removed on 2026-08-29 by owner decision.
// Its absence is now asserted by the registry-derived declared/present comparison
// above and by the retired_workflows list in the workflow contract registry.
