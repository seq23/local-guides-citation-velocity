...
for (const token of ['data/report_fixes/agent_runs/**/agent_run_manifest.json', 'npm run release:daily-citation-intelligence:preview', 'npm run release:velocity-intake', 'git push origin HEAD:main', 'contents: write', 'node-version: "24"']) if (!has(wf, token)) errors.push(`${workflowRel}:missing:${token}`);
