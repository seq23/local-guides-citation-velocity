...
for(const token of ['data/report_fixes/agent_runs/**','cron: "15 18 * * 1-5"','npm run release:velocity-intake','git push origin HEAD:main','contents: write','node-version: "24"']){if(!has(wf,token))errors.push(`${workflowRel}:missing:${token}`);}
...
for(const script of ['validate:agent-run-intake','citation:prepare-velocity-intake','release:velocity-intake','validate:velocity-intake-workflow']){if(!pkg.scripts||!pkg.scripts[script])errors.push(`package_missing_script:${script}`);}
...
if(!entry)errors.push('workflow_registry_missing_velocity_content_release');
 ...
 for(const token of ['push:agent_runs','schedule:weekday-after-noon-ct','workflow_dispatch'])if(!(entry.triggers||[]).includes(token))errors.push(`workflow_registry_velocity_missing_trigger:${token}`);
 ...
 for(const consumed of ['Twin Agent artifacts','social/public backlog'])if(!JSON.stringify(entry.consumes||[]).includes(consumed))errors.push(`workflow_registry_velocity_missing_consume:${consumed}`);
}
