...
for (const token of ['data/report_fixes/agent_runs/**/agent_run_manifest.json', 'npm run release:daily-citation-intelligence:preview', 'npm run release:velocity-intake', 'git push origin HEAD:main', 'contents: write', 'node-version: "24"', 'ALLOW_SOCIAL_FALLBACK_RELEASE: "0"']) if (!has(wf, token)) errors.push(`${workflowRel}:missing:${token}`);
...
for (const script of ['validate:agent-run-intake','citation:prepare-velocity-intake','citation:apply-html-report-contract','validate:html-report-contract','strategy:opportunities','strategy:release-queue','release:velocity-intake','validate:velocity-intake-workflow']) if (!pkg.scripts || !pkg.scripts[script]) errors.push(`package_missing_script:${script}`);
...
if (!entry) errors.push('workflow_registry_missing_velocity_content_release');
  ...
  for (const cmd of ['npm run release:velocity-intake','npm run build','npm run validate:release']) if (!(entry.commands || []).includes(cmd)) errors.push(`workflow_registry_velocity_missing_command:${cmd}`);
