...
for (const required of ['validate.yml', 'velocity_content_release.yml', 'velocity_full_rebuild.yml', 'deploy-distribution.yml', 'postdeploy_public_audit.yml']) {
  if (!files.includes(required)) errors.push(`missing:${required}`);
}

const content = fs.readFileSync(path.join(dir, 'velocity_content_release.yml'), 'utf8');
const validate = fs.readFileSync(path.join(dir, 'validate.yml'), 'utf8');
...
if (!/release:velocity-intake/.test(content) || !/git push origin HEAD:main/.test(content)) errors.push('velocity_content_release:not-autonomous');
if (!/data\/report_fixes\/agent_runs\/\*\*/.test(content)) errors.push('velocity_content_release:agent-run-path-trigger-missing');
...
if (!/release:ci-validate/.test(validate)) errors.push('validate.yml:central-command');
if (!/include-hidden-files:\s*true/.test(validate)) errors.push('validate.yml:validated-artifact-must-include-hidden-files');
