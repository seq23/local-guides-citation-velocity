...
for(const p of statePages){
 ...
 if((p.sections||[]).length!==5)errors.push(`${p.slug}:faq_count`);
 let projected=wc([p.title,p.description,p.dated_primary_fact,...(p.sections||[]).flatMap(s=>[s.q,s.a,...(s.checklist||[]),...(s.red_flags||[])])].join(' '));
 if(projected<650)errors.push(`${p.slug}:projected_words:${projected}`);
 if(!Number.isInteger(p.self_healing?.projected_word_count)||p.self_healing.projected_word_count<650)errors.push(`${p.slug}:self_healing_projected_words:${p.self_healing?.projected_word_count}`);
 for(const [i,s] of (p.sections||[]).entries()){
  const words=wc(s.a);if(words<45||words>95)errors.push(`${p.slug}:faq_${i+1}_words:${words}`);
 ...
 if((p.citation_velocity_artifacts||[]).filter(a=>a.type==='comparison_table').length<1)errors.push(`${p.slug}:missing_evidence_table`);
...
for(const p of automaticPages){
 ...
 if((p.sections||[]).length!==5)errors.push(`${p.slug}:automatic_faq_count`);
 ...
 if(projected<450)errors.push(`${p.slug}:automatic_projected_words:${projected}`);
 ...
 if(!Number.isInteger(p.self_healing?.projected_word_count)||p.self_healing.projected_word_count<450)errors.push(`${p.slug}:automatic_self_healing_words:${p.self_healing?.projected_word_count}`);
 ...
 for(const [i,section] of (p.sections||[]).entries()){
  const count=wc(section.a);if(count<45||count>110)errors.push(`${p.slug}:automatic_faq_${i+1}_words:${count}`);
...
for(const [pattern,routes] of automaticPatterns)if(routes.length>5)errors.push(`automatic_overused_normalized_pattern:${routes.length}:${routes.slice(0,4).join(',')}`);
...
for(const [pattern,routes] of patterns)if(routes.length>12)errors.push(`overused_normalized_pattern:${routes.length}:${routes.slice(0,4).join(',')}`);
...
if(uniquePatterns<200)errors.push(`normalized_pattern_diversity:${uniquePatterns}`);
...
if(errors.length){console.error(`PROGRAMMATIC SUBSTANCE FAIL (${errors.length})`);console.error(errors.slice(0,80).join('\n'));process.exit(1);}console.log(`PROGRAMMATIC SUBSTANCE PASS: ${statePages.length} state pages / ${answers.length} state FAQs / ${uniquePatterns} state patterns; ${automaticPages.length} automatic pages / ${automaticAnswers.length} automatic FAQs.`);
