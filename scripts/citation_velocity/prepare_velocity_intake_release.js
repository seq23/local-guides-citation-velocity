...
/**
 * The rule lives in scripts/lib/recommendation_proof_path.js and is shared with every
 * reader of it (the acceptance compiler and the citation-agent-fix trace). Three
 * copies of this decision existed on 2026-09-10; there is now one. See that module for
 * why, and scripts/validators/validate_shared_proof_path_resolution.js for the guard
 * that stops a fourth from appearing.
 *
 * The producer supplies its own "is this still proven here?" grader - the ledger
 * truthfulness auditor - so the readers do not inherit it as a dependency.
 */
function resolveProofPath(prior, record, computed) {
  return resolveProofPointer(prior, record, computed, {
    root: ROOT,
    isStillProvenAt: (renderedPath, required_markers) => !auditFix({ renderedPath, required_markers }).reason
  });
}
