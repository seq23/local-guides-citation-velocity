...
try{
 ...
 if(!fs.existsSync(idxPath))fail(errors,'fanout_shard_index_missing');
   ...
   for(const s of idx.shards||[]){const abs=path.join(root,s.path);if(!fs.existsSync(abs)){fail(errors,`shard_missing:${s.path}`);continue;}const buf=fs.readFileSync(abs);if(buf.length!==Number(s.byte_count))fail(errors,`shard_size:${s.path}`);if(sha(buf)!==s.sha256)fail(errors,`shard_hash:${s.path}`);let d;try{const raw=(s.compression==='gzip'||String(s.path).endsWith('.gz'))?zlib.gunzipSync(buf):buf;if(s.uncompressed_byte_count&&raw.length!==Number(s.uncompressed_byte_count))fail(errors,`shard_uncompressed_size:${s.path}`);d=JSON.parse(raw.toString('utf8'));}catch(e){fail(errors,`shard_json:${s.path}:${e.message}`);continue;}if((d.records||[]).length!==Number(s.record_count))fail(errors,`shard_count:${s.path}`);for(const r of d.records||[]){count++;if(!r.opportunity_id)fail(errors,`shard_missing_id:${s.path}`);else if(ids.has(r.opportunity_id))fail(errors,`shard_duplicate_id:${r.opportunity_id}`);else ids.add(r.opportunity_id);}agg.push(`${s.part}:${s.record_count}:${s.sha256}:${s.first_id}:${s.last_id}`);}
